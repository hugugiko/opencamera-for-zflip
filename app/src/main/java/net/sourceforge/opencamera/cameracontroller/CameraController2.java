...
import net.sourceforge.opencamera.preview.VideoMirrorer;
...
public class CameraController2 extends CameraController {
    ...
    private VideoMirrorer videoMirrorer;
        ...
        if( extension_characteristics == null ) {
            ...
            if( camera_features.supported_extensions != null ) {
                for(int extension : camera_features.supported_extensions) {
                    ...
                    List<CameraController.Size> extension_picture_sizes = camera_features_cache.extension_picture_sizes_map.get(extension);
                    List<CameraController.Size> extension_preview_sizes = camera_features_cache.extension_preview_sizes_map.get(extension);
            ...
            Map<Integer, List<CameraController.Size>> extension_picture_sizes_map = new Hashtable<>();
            Map<Integer, List<CameraController.Size>> extension_preview_sizes_map = new Hashtable<>();
            ...
            if( extensions != null ) {
                ...
                for(int extension : extensions) {
                    ...
                    try {
                        ...
                        List<android.util.Size> extension_picture_sizes_android = extension_characteristics.getExtensionSupportedSizes(extension, ImageFormat.JPEG);
                        List<CameraController.Size> extension_picture_sizes = new ArrayList<>();
                        for(android.util.Size sz : extension_picture_sizes_android) {
                            extension_picture_sizes.add(new CameraController.Size(sz.getWidth(), sz.getHeight()));
                        }
                        if( MyDebug.LOG )
                            Log.d(TAG, "    extension_picture_sizes: " + extension_picture_sizes);
                        boolean has_picture_resolution = updatePictureSizesForExtension(camera_features.picture_sizes, extension_picture_sizes, extension);

                        List<android.util.Size> extension_preview_sizes_android = extension_characteristics.getExtensionSupportedSizes(extension, SurfaceTexture.class);
                        List<CameraController.Size> extension_preview_sizes = new ArrayList<>();
                        for(android.util.Size sz : extension_preview_sizes_android) {
                            extension_preview_sizes.add(new CameraController.Size(sz.getWidth(), sz.getHeight()));
                        }
                        if( MyDebug.LOG )
     ...
     */
    private boolean updatePictureSizesForExtension(List<CameraController.Size> picture_sizes, List<CameraController.Size> extension_picture_sizes, int extension) {
        boolean has_picture_resolution = false;
        for(CameraController.Size size : picture_sizes) {
            if( extension_picture_sizes.contains(size) ) {
                if( MyDebug.LOG ) {
     ...
     */
    private boolean updatePreviewSizesForExtension(List<CameraController.Size> preview_sizes, List<CameraController.Size> extension_preview_sizes, int extension) {
        boolean has_preview_resolution = false;
        for(CameraController.Size size : preview_sizes) {
            if( extension_preview_sizes.contains(size) ) {
                if( MyDebug.LOG ) {
...
//      Frame duration is in nanoseconds.  Using min to be safe.
        ...
        try {
            ...
            synchronized( background_camera_lock ) {
                if( video_recorder != null ) {
                    ...
                    // Use the original preview size or video size?
                    // Usually preview_width/height are set to the video resolution in video mode
                    this.videoMirrorer = new net.sourceforge.opencamera.preview.VideoMirrorer(video_recorder_surface, this.preview_width, this.preview_height, current_video_rotation);
                    video_recorder_surface = this.videoMirrorer.getCameraSurface();
                }
                else {
                    if( videoMirrorer != null ) {
                        videoMirrorer.stop();
    ...
    private int current_video_rotation;

    @Override
    public void initVideoRecorderPostPrepare(MediaRecorder video_recorder, boolean want_photo_video_recording, int video_width, int video_height, int rotation) throws CameraControllerException {
        if( MyDebug.LOG )
            Log.d(TAG, "initVideoRecorderPostPrepare: " + video_width + "x" + video_height + " rotation: " + rotation);
        this.current_video_rotation = rotation;
        if( camera == null ) {
            Log.e(TAG, "no camera");
            throw new CameraControllerException();
        }
        ...
        try {
            ...
            closeCaptureSession();
            if( this.videoMirrorer != null ) {
                this.videoMirrorer.stop();
            }

            // Get actual recorder surface
            Surface recorder_surface = video_recorder.getSurface();

            // Create mirrorer with actual video dimensions and rotation
            this.videoMirrorer = new net.sourceforge.opencamera.preview.VideoMirrorer(recorder_surface, video_width, video_height, rotation);
            this.video_recorder_surface = this.videoMirrorer.getCameraSurface();
