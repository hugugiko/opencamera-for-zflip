...
public class MainUI {
    ...
    private void layoutUI(boolean popup_container_only) {
        ...
        else if( main_activity.isFreeRotationEnabled() ) {
            // 自由旋转模式：直接使用 current_orientation
            relative_orientation = current_orientation;
            if( MyDebug.LOG ) {
                Log.d(TAG, "    free rotation mode");
                Log.d(TAG, "    current_orientation = " + current_orientation);
                Log.d(TAG, "    relative_orientation = " + relative_orientation);
            }
        }
        ...
        final int ui_rotation;
        if( main_activity.isFreeRotationEnabled() ) {
            // 自由旋转模式：UI 跟随设备方向旋转
            ui_rotation = relative_orientation;
        } else {
            // lock_to_landscape 模式：原有逻辑
        ...
        if( !popup_container_only ) {
            setTakePhotoIcon();
            setSwitchCameraContentDescription();
        }
    ...
    public void setSwitchCameraContentDescription() {
        if( MyDebug.LOG )
            Log.d(TAG, "setSwitchCameraContentDescription() (custom: free rotation)");
        ImageButton view = main_activity.findViewById(R.id.switch_camera);
        if( view != null ) {
            view.setContentDescription("自由旋转切换");
            view.setImageResource(R.drawable.baseline_rotate_right_white_48);
        }
    }
    ...
    public void onOrientationChanged(@SuppressWarnings("ParameterCanBeLocal") int orientation) {
        ...
        if( !MainActivity.lock_to_landscape && !main_activity.isFreeRotationEnabled() )
            return;
        // if locked to landscape or free rotation, we need to handle the orientation change ourselves
